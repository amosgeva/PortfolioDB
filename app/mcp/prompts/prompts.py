...
def _morning_brief(style: str = "terse") -> str:
...
def _analyze_concentration_risk(top_n: int = 10) -> str:
...
def _review_recent_activity(window: str = "1w") -> str:
...
def _compare_methods_brief() -> str:
...
def _existing_position(pos: dict[str, Any] | None) -> dict[str, float]:
    """Quantity, market value, weight and open cost held now; zeros when none."""
    if not pos:
        return {"qty": 0.0, "mv": 0.0, "weight": 0.0, "cost": 0.0}
    return {
        "qty": float(pos["qty"]),
        "mv": float(pos.get("market_value") or 0.0),
        "weight": float(pos.get("weight_pct") or 0.0),
        "cost": float(pos.get("open_cost") or 0.0),
    }


def _after_trade(
    side_u: str, qty: float, price: float | None, existing: dict[str, float]
) -> dict[str, float | None]:
    """Approximate position figures once the proposed trade has filled."""
    notional = (qty * price) if price is not None else None
    if side_u == "BUY":
        new_qty = existing["qty"] + qty
        new_cost = existing["cost"] + (notional or 0.0)
    else:  # SELL
        new_qty = existing["qty"] - qty
        # SELL doesn't change open_cost directly — engine re-computes.
        new_cost = existing["cost"] * (new_qty / existing["qty"]) if existing["qty"] else 0.0
    new_mv = (new_qty * price) if price is not None else None
    delta_mv = (new_mv - existing["mv"]) if new_mv is not None else None
    return {"notional": notional, "qty": new_qty, "cost": new_cost, "mv": new_mv, "delta_mv": delta_mv}
...
def _pre_trade_check(
    ...
    existing = _existing_position(pos)
    after = _after_trade(side_u, qty, price, existing)
    notional = after["notional"]

    # Approx new weight assuming total MV moves by delta.
    new_total_mv = kpis["market_value"] + (after["delta_mv"] or 0.0)
    new_weight = (
        (after["mv"] / new_total_mv * 100.0)
        if (after["mv"] is not None and new_total_mv)
        else None
    )
    ...
    return (
        ...
        f"## Current position\n"
        f"- qty: {existing['qty']:.4f}  cost basis: {_money(existing['cost'])}  "
        f"market value: {_money(existing['mv'])}  weight: {_pct(existing['weight'])}\n\n"
        f"## After this trade\n"
        f"- qty: {after['qty']:.4f}\n"
        f"- approx cost basis: {_money(after['cost'])}\n"
        f"- approx market value: {_money(after['mv'])}\n"
...
def _facts_block(facts: dict[str, Any] | None) -> str:
    if not facts:
        return "_no profile data on file_"
    return (
...
def _metrics_block(metrics: dict[str, Any] | None) -> str:
    if not metrics:
        return "_no metrics on file_"

    def ratio_pct(key: str, signed: bool = False) -> str:
        return _pct((metrics.get(key) or 0) * 100, signed=signed)
    ...
    return (
        ...
        f"- Margins: gross {ratio_pct('gross_margin')}  · "
        f"op {ratio_pct('operating_margin')}  · "
        f"net {ratio_pct('net_margin')}\n"
        f"- ROE: {ratio_pct('return_on_equity')}  · "
        f"ROA: {ratio_pct('return_on_assets')}\n"
        f"- Revenue growth: {ratio_pct('revenue_growth', signed=True)}  · "
        f"EPS growth: {ratio_pct('earnings_growth', signed=True)}\n"
    )


def _fundamentals_brief(symbol: str) -> str:
    sym = symbol.upper()
...
def _drawdown_review(top_n: int = 5, since: date | None = None) -> str:
...
def register(mcp: FastMCP) -> None:
    """Attach every prompt to `mcp`.

    The wrappers carry only the MCP metadata; each body is a module-level
    `_<name>` function above, so it can be read and exercised on its own.
    """
    ...
    def morning_brief(style: str = "terse") -> str:
        ...
        return _morning_brief(style=style)
    ...
    def analyze_concentration_risk(top_n: int = 10) -> str:
        return _analyze_concentration_risk(top_n=top_n)
    ...
    def review_recent_activity(window: str = "1w") -> str:
        return _review_recent_activity(window=window)
    ...
    def pre_trade_check(
        ...
        return _pre_trade_check(symbol=symbol, side=side, qty=qty, price=price)
    ...
    def fundamentals_brief(symbol: str) -> str:
        return _fundamentals_brief(symbol=symbol)
    ...
    def drawdown_review(top_n: int = 5, since: date | None = None) -> str:
        return _drawdown_review(top_n=top_n, since=since)
